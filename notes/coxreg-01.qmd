...
먼저 첫 번째 생존 시간 $x_1=6$ 에 대한 부분 가능도함수를 구해보자. 이 경우 위험그룹 $R(6)$ 는 모든 환자이며 부분 가능도함수는 다음과 같다.
...
두 번째 생존 시간 $x_2=7$ 은 중도절단이므로 부분 가능도함수에 포함되지 않는다.

세 번째 생존 시간 $x_3=10$ 에 대한 부분 가능도함수는 다음과 같다. 이때 위험그룹 $R(10)$ 는  3, 4, 5, 6 번 환자이다.
...
$$
 이제 위의 결과를 모두 곱하면 부분 가능도함수 $L_p(\beta)$ 를 구할 수 있다. 아레 부분 가능도함수를 유도하는 경우 @eq-coxex1 에서 정의된 설명변수를 사용하였다.
